...
class ChatMessage(BaseModel):
    """Chat message model for dashboard communication."""

    message: str = Field(..., description="User message content")
    context: Optional[List[Dict[str, Any]]] = Field(default=None, description="Previous conversation context")
    session_id: Optional[str] = Field(default=None, description="Chat session identifier")


class ChatResponse(BaseModel):
    """Response from AI orchestrator chat."""
...
class PRD(BaseModel):
    ...
    id: str = Field(default_factory=lambda: str(uuid4()), description="PRD unique identifier")
    title: str = Field(..., description="PRD title")
    content: str = Field(..., description="PRD content in markdown format")
    status: PRDStatus = Field(default=PRDStatus.DRAFT, description="PRD status")
    requirements: str = Field(..., description="Original requirements that generated this PRD")
    validation_questions: Optional[List[str]] = Field(default=None, description="Questions asked during validation")
    validation_answers: Optional[List[str]] = Field(default=None, description="Answers provided during validation")
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Creation timestamp")
    updated_at: datetime = Field(default_factory=datetime.utcnow, description="Last update timestamp")
    created_by: Optional[str] = Field(default=None, description="User who created the PRD")


class PRDList(BaseModel):
    """List of PRDs."""

    prds: List[PRD] = Field(..., description="List of PRDs")
...
class PRDGenerateRequest(BaseModel):
    ...
    requirements: str = Field(..., description="Project requirements description")
    context: Optional[List[Dict[str, Any]]] = Field(default=None, description="Additional context from chat")


class PRDValidationRequest(BaseModel):
    """Request to validate a PRD."""

    prd_id: str = Field(..., description="PRD to validate")
    feedback: Optional[str] = Field(default=None, description="User feedback on the PRD")


class PRDApprovalRequest(BaseModel):
    """Request to approve a PRD."""

    prd_id: str = Field(..., description="PRD to approve")
