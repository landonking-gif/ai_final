...
class SubagentLifecycleManager:
    ...
    def _create_llm_adapter(self) -> LLMAdapter:
        ...
        Priority:
        1. OpenClaw (if use_openclaw=True and available)
        ...
        if getattr(self.config, 'use_openclaw', True):
            try:
                from adapters.llm.openclaw import OpenClawAdapter, OPENCLAW_AVAILABLE
                
                if OPENCLAW_AVAILABLE:
                    return OpenClawAdapter(
                        gateway_url=getattr(self.config, 'openclaw_gateway_url', 'ws://openclaw:18789'),
                        model=self.config.llm_model or "ollama/deepseek-r1:14b",
                        temperature=self.config.llm_temperature,
                        max_tokens=self.config.llm_max_tokens,
                    )
                else:
                    logger.warning("OpenClaw not available, falling back to other provider")
            except Exception as e:
                logger.warning(f"OpenClaw initialization failed: {e}, falling back")
        ...
        elif provider in ("local", "openclaw"):
    ...
    async def execute_task(self, request: SubagentExecuteRequest) -> SubagentResponse:
        ...
        try:
            # Execute with timeout using asyncio.wait_for instead of anyio
            response = await asyncio.wait_for(
                self._execute_with_llm(context, request),
                timeout=timeout
            )
